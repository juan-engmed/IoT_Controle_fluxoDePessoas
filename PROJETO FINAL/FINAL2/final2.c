...
//Variaveis matriz LED
int i;
int j;
int k;
char a_p[]={1,2,4,8,16,32,64,128};
char a_n[][8]={
...
while (1)
      ...
      delay_ms(200); 
        ...
        while(cont >= 6)
            ...
            if (cont == 2)
            {
            sprintf(aviso,"MANTER DISTANCIA");
            lcd_clear();
            lcd_puts(aviso);
            };
            ...
            if (cont >= 6)
            {
            sprintf(aviso,"FAVOR RETIRAR-SE");
            lcd_clear();
            lcd_puts(aviso);
            };
            
            lcd_gotoxy(7,1);
            sprintf(lotacao,"%d", cont);
            lcd_puts(lotacao);
      }      
